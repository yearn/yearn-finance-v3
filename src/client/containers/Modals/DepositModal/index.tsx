...
import { TokensSelectors, VaultsSelectors, VaultsActions } from '@store';
...
export const DepositModal: FC<DepositModalProps> = ({ onClose, ...props }) => {
  ...
  const [selectedSellTokenAddress, setSelectedSellTokenAddress] = useState('');
  ...
  useEffect(() => {
    setSelectedSellTokenAddress(selectedVault?.token.address ?? '');
  }, [selectedVault]);
