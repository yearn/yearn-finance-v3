...
const ledgerIframeWallet: WalletModule = {
  ...
  wallet: async () => {
    ...
    return {
      getProviderName: () => 'Iframe',
      provider: provider,
      interface: {
        name: 'Iframe',
        loading: async () => false, //Avoid loading
        address: {
          onChange: (func) => {
            const handlerAccount = (accounts: string[]) => func(accounts && accounts[0]);
            web3Provider
              .getSigner()
              .getAddress()
              .then((addr) => func(addr));
            provider.on('accountsChanged', handlerAccount);
          },
        },
        network: {
          onChange: (func) => {
            const handlerChain = (chainID: number) => func(chainID);
            web3Provider.getNetwork().then((network) => func(network.chainId));
            web3Provider.on('networkChanged', handlerChain);
            web3Provider.on('chainChanged', handlerChain);
          },
        },
        balance: {
          get: async () => ethers.utils.formatEther(await web3Provider.getSigner().getBalance()),
        },
      },
    };
  },
...
export default ledgerIframeWallet;
