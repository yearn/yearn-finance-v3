...
export const VaultsActions = {
  setSelectedVaultAddress: createAction<{ vaultAddress: string }>('vaults/setSelectedVaultAddress'),

  initiateSaveVaults: createAsyncThunk<void, string | undefined, ThunkAPI>(
    'vaults/initiateSaveVaults',
    async (_arg, { dispatch }) => {
      await dispatch(VaultsActions.getVaults());
    }
  ),

  getVaults: createAsyncThunk<
  ...
  approveVault: createAsyncThunk<void, { vaultAddress: string }, ThunkAPI>(
    'vaults/approveVault',
  ...
  depositVault: createAsyncThunk<void, { vaultAddress: string; amount: BigNumber }, ThunkAPI>(
    'vaults/depositVault',
    async ({ vaultAddress, amount }, { extra, getState, dispatch }) => {
      ...
      const approved = new BigNumber(userTokenData.allowancesMap[vaultAddress]).gt(0);
      if (!approved) {
        await dispatch(VaultsActions.approveVault({ vaultAddress }));
      }
  ...
  withdrawVault: createAsyncThunk<void, { vaultAddress: string; amount: BigNumber }, ThunkAPI>(
